...
export class Index extends React.Component {
  ...
  constructor(props) {
    ...
    if (typeof Window !== "undefined") {
      ...
      if (
        vID &&
        vID !== "" &&
        this.props.channels.filter(v => v.videoId === vID).length > 0
      ) {
  ...
  componentDidMount() {
    ...
    Mousetrap.bind("n", () => {
      ...
      if (this.props.channel !== "") {
        let i = localChannels.findIndex(
          el => el.videoId === this.props.channel
        );
    ...
    Mousetrap.bind("p", () => {
      ...
      if (this.props.channel !== "") {
        ...
        if (i === 0) {
          this.props.dispatch(
            updateChannel(localChannels[localChannels.length - 1].videoId)
          );
        } else {
          this.props.dispatch(updateChannel(localChannels[i - 1].videoId));
        }
    ...
    // Random
    Mousetrap.bind("r", () => {
      const arr = this.currentChannels.filter(
        c => c.videoId !== this.props.channel
      );
  ...
  get currentChannel() {
    return (
      this.props.channels.filter(c => c.videoId === this.props.channel)[0] || {}
    );
  }
  ...
  get currentChannels() {
    if (this.props.filter === "All") {
      ...
      return this.props.channels.filter(
        c => c.genres.indexOf(this.props.filter) > -1
      );
    }
  }
  ...
  get pageTitle() {
    return this.props.filter === "All"
      ? null
      : `${this.props.filter.replace(/\b\w/g, l =>
          l.toUpperCase()
        )} radio | mmmusic. 24/7 music to focus, work and relax`;
  }
